...
def test_student_search_and_pagination(client):
    ...
    search_response = client.get("/students?search=Student+03")

    assert search_response.status_code == 200
    assert b"Student 03" in search_response.data
    assert b"Student 04" not in search_response.data

    first_page = client.get("/students?page=1&per_page=5")
    ...
    second_page = client.get("/students?page=2&per_page=5")
...
def test_course_search_and_pagination(client):
    ...
    search_response = client.get("/courses?search=Course+03")

    assert search_response.status_code == 200
    assert b"Course 03" in search_response.data
    assert b"Course 04" not in search_response.data

    first_page = client.get("/courses?page=1&per_page=5")
    ...
    second_page = client.get("/courses?page=2&per_page=5")
